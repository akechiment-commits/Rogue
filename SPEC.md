...
- **石像**: `dg.statues`。**ダメージ／有害効果**（投擲・矢・杖弾・爆発など）で破壊→ややレアなアイテム＋**そのフロアの出現敵プールから1体（出現レベル+1、最大3）**。通行不可。**移動で突っ込む／Z・Enterで正面調べは「石像がある。」表示のみ・ターン消費なし**（近接では壊さない）。場所替え・テレポ等の非ダメージでは壊れない。実装: `fixtures.hitStatueWithAction` / `wandEffectBreaksStatue`
