...
- アイテムモドキ（`itemMimic`）は出現時から `type:"item_mimic"` の床アイテム「アイテムモドキ」として待機し、移動・攻撃をしない。プレイヤーが拾おうとした瞬間に偽アイテムを消して正体を現し、その場で攻撃する。化けている間は描画・占有判定とも床アイテムを優先し、起動したターンの敵フェーズでは追加行動しない。偽アイテムは通常の床アイテム処理の対象で、炎の杖・爆発などで破壊された場合は本体も消滅し、吹き飛ばし等で移動した場合は本体も床アイテムに追従する。
