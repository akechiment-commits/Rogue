...
オブジェクト用のスタイル3タイルは、階段が2/3、泉が31、大箱が38（鍵・金具のない単純な木箱）、魔方陣が74、骨が107、石像が129、固定転送陣が130。風穴（128）は方向を示す現行の矢印描画を維持する。アイテムカテゴリの共通画像は仮想ID 4001〜4003を使い、薬・空き瓶・ペンの冒険内ランダム画像より優先して表示する。
