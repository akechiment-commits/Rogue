...
- **石像**: `dg.statues`。**ダメージ／有害効果**を与える行動で破壊→ややレアなアイテム＋**そのフロアの出現敵プールから1体（出現レベル+1、最大3）**。通行不可（上に乗れない）。投擲・矢・杖弾・近接で当たる。場所替え・テレポ・飛びつき・穴掘り・祝福／呪い杖など**非ダメージ**では壊れない（「効果がなかった」）。実装: `fixtures.hitStatueWithAction` / `wandEffectBreaksStatue`
