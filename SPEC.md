...
- **風穴**: `dg.vents`。中心±2 の 5×5 で飛び道具の進行方向を風向きに上書き（遠投中も同様）
- **石像**: `dg.statues`。ダメージ／状態異常を与えるアクションで破壊→ややレアなアイテム＋**そのフロアの出現敵プールから1体（出現レベル+1、最大3）**。場所替えのみでは壊れない。敵は能動破壊しないが弾の射線なら割れる
- **固定転送**: `pentacles` の `kind:"fixed_portal"`。生成時ペア固定。ペンのポータルとは非接続
