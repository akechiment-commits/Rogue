...
- 盗投士（`stealthrower`）は、隣接時にプレイヤーのアイテムを1個だけ盗んで手持ちに入れ、投げるか倒されるまで追加では盗まず、次ターン以降に一直線上から投げる。護盗の鎧で盗み・投擲を防げる。射線上に壁などの遮蔽物がある場合も投擲を実行し、遮蔽物の手前へ落とす。敵・石像などに命中した投擲物は消滅し、壁などに遮られて効果が発生しなかった場合だけ手前の地面に落ちる。倒されると手持ちのアイテムを落とす。
