...
敵のうち、既存の汎用画像では名前との不一致が大きかったものはスタイル3専用ID 145〜158へ分離している。対応は `monsters.js` と `tilesetMap.js` を参照する（ゴースト、かいりきベア、バリア蟹、アーチャー、ゴールドタイガー、罠師、足払い鬼、ダークネス、盗投士／コソドロ、岩砕き、ガーディアン、解装士、ウィンドメイジ、マイナーダイミョウ）。
