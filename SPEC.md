...
- **泉**: 「飲む」約1%、「浸す」約0.8%で発動。叶うと泉は必ず干上がる
- **願いの杖** (`effect: wish`): 常に1回。充填・合成で回数増加不可。振ると願いUI
- **願いの壺** (`potEffect: wish_pot`): 物を入れると願いUI。捧げた物と壺は消滅（1回限り）
- UI:
  - **直接入力**: 図鑑未登録の道具も可（最上位合成品・願いアイテム自体は拒否）
  - **道具**: 図鑑（セーブ＋今回の探索）に載った道具のみ。デバッグ魔法同様のカテゴリ選択
  - **恩恵**: 全回復（満腹含む）／全識別／フロアを見通す／レベル+3／敵の全滅（通常撃破扱い・経験値とドロップあり）
...
- `tryPortrait(key)` ：4秒クールタイム内は切り替えしない（歩行・攻撃など）
- `PORTRAIT_SETS` オブジェクトで各 key → ファイル名の配列（ランダム選択）
- 優先度：死亡 > 被ダメージ > レベルアップ > 状態異常 > アイテム使用 > 歩行 > 攻撃
