...
export const ITEMS = [
  ...
  { name:"防具強化の巻物",   type:"scroll", effect:"armor_up",           rarity:"B", weight:4,  sellPrice:800,  desc:"選んだ防具・または＋値のつく指輪の＋値を1上げる。",  tile:18 },
  ...
  { name:"炎の巻物",         type:"scroll", effect:"flame",               rarity:"B", weight:4,  sellPrice:550,  desc:"視界内の敵に炎ダメージ(30-40)。油まみれの対象は2倍。祝福でフロア全体。呪いで自分にも同ダメージ。", tile:18 },
  ...
  { name:"金縛りの巻物",     type:"scroll", effect:"bind",                rarity:"B", weight:4,  sellPrice:600,  desc:"周囲8マスの敵を金縛りにする。祝福で視界内全体。呪いで自分が20ターン金縛り。", tile:18 },
  ...
  { name:"売却の巻物", type:"scroll", effect:"sell_item",         rarity:"B", weight:4,  sellPrice:600,
    desc:"持ち物から1つ選んで換金する。祝福：2倍の金額。呪い：半額。", tile:18 },
  { name:"変換の巻物", type:"scroll", effect:"transform_item",    rarity:"B", weight:4,  sellPrice:700,
    desc:"持ち物から1つ選んで別のアイテムに変える。祝福：レアリティが高いものに変化。呪い：レアリティが低いものに変化。", tile:18 },
  { name:"錬成の巻物", type:"scroll", effect:"forge_item",        rarity:"A", weight:2,  sellPrice:2000,
    desc:"持ち物の武器か防具を1つ選んでランダムな能力を付与する。祝福：強力な能力。呪い：役に立たない能力。", tile:18 },
...
export const WANDS = [
  ...
  { name:"金縛りの杖",     type:"wand", effect:"paralyze",  charges:5, rarity:"A", weight:2,  sellPrice:500, desc:"振ると対象を金縛りにする。何かアクションを受けるまで動けなくなる。祝福：2回アクションが必要な強金縛り。呪い：対象が200ターン状態異常（金縛り・眠り・鈍足）を防ぐ力を得る。水の瓶に当てると金縛りの薬になる。", tile:24 },
  ...
  { name:"呪いの杖",       type:"wand", effect:"curse_wand",charges:1, rarity:"B", weight:4,  sellPrice:1500, desc:"振ると対象のアイテムを呪う。壊すと周囲のアイテム全てを呪う。",                         tile:24 },
  ...
  { name:"物知りの杖",     type:"wand", effect:"sage",          charges:4, rarity:"B", weight:4,  sellPrice:700,  desc:"アイテム・大箱に当てると識別する。敵に当てるとHP・攻撃力・防御力を表示する。壁に跳ね返って自分に当たると手持ちから1個ランダム識別。呪い：対象が未識別に戻る。", tile:24 },
];

/* ===== BIG BOX TYPES ===== */
export const BB_TYPES = [
  { kind: "synthesis", name: "合成の大笥", cap: () => 2,          weight: 2, desc: "2つのアイテムを合成する。武器同士・防具同士なら能力を引き継ぐ。杖同士ならチャージを合算。ペン同士なら合算。杖と武器・防具の組み合わせでは装備に杖の能力が宿る異種合成もある。組み合わせによっては別のアイテムに変化する特殊合成が発生することもある。" },
  { kind: "change",    name: "変化の大箱", cap: () => rng(2, 4),  weight: 1, desc: "入れたアイテムがランダムな別のアイテムに変化する。何に変わるかは開けるまで不明。キーアイテムは変化しない。" },
  { kind: "enhance",   name: "強化の大箱", cap: () => rng(1, 2),  weight: 1, desc: "武器・防具の＋値を1上げる。力・守り・命の指輪の＋値も増やせる。壺の容量を1増やす。他のアイテムには効果がない。" },
  { kind: "satiety",   name: "満腹の大箱", cap: () => rng(2, 4),  weight: 1, desc: "食料のサイズを1段階大きくする。生なら最大で超特大、調理済みなら最大で爆盛りになる。食料以外には効果がない。" },
  { kind: "refill",    name: "充填の大箱", cap: () => rng(1, 3),  weight: 1, desc: "杖・ペン・魔法のマーカーの使用回数をランダムに回復する。" },
  { kind: "identify",  name: "鑑定の大箱", cap: () => rng(3, 5),  weight: 1, desc: "入れたアイテムを識別する。薬・巻物・杖の見た目名が判明し、武器・防具の呪い状態も分かる。" },
  { kind: "split",     name: "分裂の大箱", cap: () => 1,          weight: 1, rare: true, desc: "【レア】入れたアイテムを複製する。＋値・矢の数は半減する。金貨とキーアイテムは分裂しない。" },
  { kind: "bless",     name: "祝福の大箱", cap: () => rng(1, 2),  weight: 1, rare: true, desc: "【レア】入れたアイテムを祝福する。壺は祝福ではなく容量が1増える。キーアイテムには効果がない。" },
  { kind: "curse",     name: "呪いの大箱", cap: () => rng(1, 2),  weight: 1, rare: true, desc: "【レア】入れたアイテムを呪う。壺は容量が1減る。食料は腐る。金貨・キーアイテムには効果がない。" },
  { kind: "scatter",   name: "拡散の大箱", cap: () => rng(3, 6),  weight: 1, desc: "入れたアイテムを同じ部屋にいる全員に投げつけ消滅させる。薬・杖・壺・矢は各種効果発動。使うたびに容量が減り壊れる。" },
...
export const POTS = [
  ...
  { name:"オリーブオイルの壺", type:"pot", potEffect:"olive",     capacity:3, rarity:"C", weight:8,  sellPrice:550,  desc:"食料を入れるとオリーブオイル漬けになる。食べると被攻撃15%回避(80ターン)。満タンでない状態で割れると周囲8マスに油が飛散し、油まみれの床と油状態(100T)を付与。油まみれなら炎ダメージ2倍。", tile:32 },
  { name:"ごま油の壺",         type:"pot", potEffect:"sesame",    capacity:3, rarity:"C", weight:8,  sellPrice:550,  desc:"食料を入れるとごま油風味になる。食べると会心率UP(80ターン)。満タンでない状態で割れると周囲8マスに油が飛散し、油まみれの床と油状態(100T)を付与。油まみれなら炎ダメージ2倍。",         tile:32 },
  { name:"バターの壺",         type:"pot", potEffect:"butter",    capacity:3, rarity:"C", weight:8,  sellPrice:550,  desc:"食料を入れるとバター風味になる。食べると満腹度減少速度半減(100ターン)。満タンでない状態で割れると周囲8マスに油が飛散し、油まみれの床と油状態(100T)を付与。油まみれなら炎ダメージ2倍。",         tile:32 },
...
export const RINGS = [
  ...
  { name: "腹持ちの指輪",   type:"ring", effect:"stomach_ring",          rarity:"B", weight:2, sellPrice:5000, tile:60, desc:"装備中、満腹度の減りが半分になる。" },
