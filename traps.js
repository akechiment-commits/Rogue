...
/** 飛んできた罠がモンスターに命中したときに効果を発動する */
export function fireTrapMonster(trap, mon, dg, ml) {
  switch (trap.effect) {
    case "explode":
      ml.push(`${trap.name}が発動！`);
      doExplosion(trap.x, trap.y, dg, null, ml, null, trap.name);
      break;
    case "sleep":
      mon.sleepTurns = (mon.sleepTurns || 0) + rng(3, 6);
      ml.push(`${trap.name}が発動！${mon.name}が眠りに落ちた！`);
      break;
    case "slow_trap":
      mon.slowTurns = (mon.slowTurns || 0) + 10;
      ml.push(`${trap.name}が発動！${mon.name}の動きが鈍くなった！`);
      break;
    case "seal_trap":
      mon.sealed = true;
      ml.push(`${trap.name}が発動！${mon.name}の魔法が封印された！`);
      break;
    case "spin": {
      const rm = dg.rooms[rng(0, dg.rooms.length - 1)];
      mon.x = rng(rm.x, rm.x + rm.w - 1);
      mon.y = rng(rm.y, rm.y + rm.h - 1);
      ml.push(`${trap.name}が発動！${mon.name}が吹き飛ばされた！`);
      break;
    }
    case "blowback_trap": {
      const _bDirs = [[-1,0],[1,0],[0,-1],[0,1]];
      const _bd = _bDirs[rng(0, 3)];
      let _bbWall = false;
      for (let i = 0; i < 10; i++) {
        const _bnx = mon.x + _bd[0], _bny = mon.y + _bd[1];
        if (_bnx < 0 || _bnx >= MW || _bny < 0 || _bny >= MH ||
            dg.map[_bny][_bnx] === T.WALL || dg.map[_bny][_bnx] === T.BWALL) {
          _bbWall = true; break;
        }
        mon.x = _bnx; mon.y = _bny;
      }
      ml.push(`${trap.name}が発動！${mon.name}が吹き飛ばされた！`);
      if (_bbWall) { mon.hp -= 10; ml.push(`${mon.name}が壁に激突！10ダメージ！`); }
      if (mon.hp <= 0) { ml.push(`${mon.name}は倒れた！`); removeMonster(dg, mon); }
      break;
    }
    case "summon_trap": {
      ml.push(`${trap.name}が発動！`);
      const _sc = rng(1, 3);
      spawnMonsters(dg, _sc, 1, mon.x, mon.y, null, { aware: true });
      ml.push(`${_sc}体の敵が現れた！`);
      break;
    }
    case "arrow_trap":
    case "poison_arrow": {
      const _ad = rng(5, 12);
      mon.hp -= _ad;
      ml.push(`${trap.name}が発動！${mon.name}に${_ad}ダメージ！`);
      if (trap.effect === "poison_arrow") {
        mon.atk = Math.max(1, Math.floor((mon.atk || 1) / 2));
        ml.push(`${mon.name}の攻撃力が半減した！`);
      }
      if (mon.hp <= 0) { ml.push(`${mon.name}は倒れた！`); removeMonster(dg, mon); }
      break;
    }
    default:
      ml.push(`${trap.name}が発動したが${mon.name}には効果がなかった！`);
      break;
  }
}
