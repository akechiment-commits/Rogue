...
export const TRAPS = [
  ...
  /* B: レア（危険） */
  { name:"地雷",           effect:"explode",       tile:25,  rarity:"C", weight:4,  desc:"踏むと周囲8マスが大爆発（敵ターン後）。敵は即死（経験値なし）、プレイヤーはHP半減（耐火で軽減）。耐火なしなら所持品1つが焼ける。\n壁・罠・大箱・床のアイテムも破壊される。爆発範囲内の地雷・時限爆弾は、爆発の種類によらず誘爆する。" },
  { name:"時限爆弾の罠",   effect:"time_bomb",     tile:73,  rarity:"B", weight:2,  desc:"踏むと4ターン後に大爆発が起きる。\n爆発は地雷と同じ威力。敵は即死（経験値なし）。プレイヤーはHP1（耐火で軽減）。耐火なしなら所持品1つが焼ける。爆発範囲内の地雷・時限爆弾は、爆発の種類によらず誘爆する。離れれば回避できる。\n作動済みの爆心地に薬液をかけると消火可能。\n作動済みは爆心地にカウントダウン表示。" },
  ...
  { name:"水鉄砲の罠",     effect:"watergun_trap",  tile:132, rarity:"C", weight:4,  desc:"踏むと水鉄砲を浴びる。ずぶ濡れになり、所持品からランダム1つに水の影響が出る。影響のない品なら無事。\n巻物・魔法書は白紙化、食料はサイズ1段階縮小、ペンはインク-1、火薬壺は保存の壺、爆弾矢・這いずり爆弾は1つ減る。\nアーマーガッパ（耐水）で防げる。" },
