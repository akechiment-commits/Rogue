...
export const FIRST_ENCOUNTER_TIPS = Object.freeze({
  ...
  item_bottle: { title: "空き瓶", trigger: "初めて空き瓶を拾う", text: ["空き瓶は泉に浸すと水になり、薬として飲んだり投げたりできる。", "空き瓶を投げると割れて消える。敵にぶつけて倒すと薬になるので、使い道を考えよう。"] },
  ...
  spring: { title: "泉", trigger: "初めて泉の上に乗る、または横から存在を調べる", text: ["泉では水を飲むか、持ち物を浸せる。結果は道具の種類によって変わる。", "利用するたびに泉が干上がることがあるので、何に使うか考えよう。"] },
  bigbox: { title: "大箱", trigger: "初めて大箱の上に乗る、または横から存在を調べる", text: ["大箱へ道具を投げ入れると、箱の種類に応じた効果が起きる。", "中身は直接取り出せない。さらに入れて容量オーバーさせるか、箱を壊して回収する。"] },
  gacha_machine: { title: "ガチャマシーン", trigger: "足元・正面・フロア一覧からガチャマシーンを調べる", text: ["1000Gを払うと景品が1個出る。ほとんどは大した物ではなく、当たりは滅多に出ない。", "Aランクは大当たり、Bランクは当たり。炎・雷・氷・穴掘り・軟化などの破壊系の杖、ダメージ系の薬、爆発で壊れて景品を1個落とす。店内の台を壊すか店外へ出すと泥棒扱いになる。"] },
  dimensional_vault: { title: "次元宝物庫", trigger: "次元宝物庫の部屋へ入る", text: ["部屋に入った瞬間から、内部のアイテムが消えるまでのカウントが始まる。", "アイテムの上に表示された残りターンを確認し、消える前に拾おう。"] },
  wandering_merchant: { title: "行商人", trigger: "行商人に話しかける", text: ["行商人とは、その場でアイテムを売買できる。", "攻撃すると敵対し、その階では泥棒扱いになる。"] },
  altar: { title: "祭壇", trigger: "祭壇を調べる", text: ["祭壇には食料だけを捧げられ、代わりに別のアイテムが返ってくる。", "同じ祭壇へ捧げた回数が増えるほど、レアな返礼品が出やすくなる。"] },
