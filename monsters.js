...
export const MONS = [
  ...
  { name: "マイナーダイミョウ", hp: 30,  atk: 17, def: 3,  exp: 42,  speed: 1,   tile: 39,  kind: "humanoid", baseKind: "itemblaster",  monLevel: 1, minFloor: 6,  maxFloor: 22, subtype: "itemblast", dungeonFloors: { beginner: null, intermediate: { min: 9, max: 15 } },
    levels: [
  ...
  { name: "ゴールドタイガー", hp: 38,  atk: 20, def: 5,  exp: 55,  speed: 1,   tile: 39, kind: "humanoid", baseKind: "trapthrower",   monLevel: 1, minFloor: 18, maxFloor: 50, subtype: "trapthrower", dungeonFloors: { intermediate: { min: 18, max: 19 } },
    levels: [
  ...
  { name: "かいりきベア", hp: 43,  atk: 22, def: 6,  exp: 62,  speed: 1,   tile: 11, kind: "humanoid", baseKind: "monsterthrow",  monLevel: 1, minFloor: 20, maxFloor: 50, subtype: "monsterthrow", dungeonFloors: { intermediate: { min: 18, max: 19 } },
    levels: [
  ...
  { name: "バリア蟹",     hp: 43,  atk: 20, def: 3,  exp: 68,  speed: 1,   tile: 54, kind: "humanoid", baseKind: "barriermage",   monLevel: 1, minFloor: 22, maxFloor: 50, barrier: 1,
    levels: [
  ...
  { name: "ほっちもぺ",   hp: 68, atk: 24, def: 18, exp: 70, speed: 1,   tile: 57, kind: "beast",   baseKind: "reflector",     monLevel: 1, minFloor: 12, maxFloor: 45, subtype: "reflector", dungeonFloors: { intermediate: { min: 11, max: 18 } },
    levels: [
...
export function monsterAI(m, dg, pl, ml, opts = {}) {
  ...
  if (m.baseKind === "im_boss_darkfog" && !_moveOnly) {
    ...
    if (!_dfSeal) {
      ...
      /* 遠距離（非隣接）& 直線LOS → 魔法弾（魔法は遠投・みかわし不適用・反射の鎧有効） */
        ...
        for (let _bd = 1; _bd < 20; _bd++) {
          ...
          if (_btx === pl.x && _bty === pl.y) {
            if (hasAbility(pl.armor, "wand_reflect")) {
              ...
              m.hp -= _rfDmg; ml.push(`魔法弾が${m.name}に命中！${_rfDmg}ダメージ！`);
              ...
              pl.hp -= _dfDmg; pl.deathCause = `${m.name}の魔法弾で`;
