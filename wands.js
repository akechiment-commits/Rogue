...
export function applyWandEffect(eff, kind, target, dx, dy, dg, p, ml, luFn, bbFn, blMult = 1, nameFn = null, collisionAtk = 0, killerMon = null, bigboxNameFn = null, sourceIsPlayer = true, breaker = null) {
  const _magicDamage = (amount, victim = target) =>
    kind === "monster" && sourceIsPlayer
      ...
      : multiplyCursedMagicDamage(amount, victim, dg);
  ...
  switch (eff) {
    ...
    case "slow": {
      ...
      if (_sCurse) {
        ...
        if (kind === "player") {
          ...
          ml.push(`体が軽くなった！(${hasteDurationLabel(p, _ht)})【呪】`);
          break;
        }
      } else {
        if (kind === "monster") {
    ...
    case "swap": {
      ...
      if (FLOOR_MOVE_KINDS.has(kind)) {
        ...
        if (isFloorOccupancyBlocked(dg, ox, oy, { ignore: kind === "stair" ? null : target, allowPlayer: true, allowMonster: true })) {
        ...
        if (kind === "stair") {
          ...
          if (!moveStairTo(dg, target, ox, oy)) {
            ml.push(`${_flName}を動かせなかった。`);
            break;
          }
    ...
    case "leap": {
      if (blMult < 1) {
        ...
        if (FLOOR_MOVE_KINDS.has(kind)) {
          ...
          const _lpd = pickFreeFloorObjectCell(dg, kind === "stair" ? null : target, p, _lpOx, _lpOy);
          if (!_lpd) { ml.push("テレポートに失敗した。"); break; }
          if (kind === "stair") {
            if (!moveStairTo(dg, target, _lpd.x, _lpd.y)) { ml.push("テレポートに失敗した。"); break; }
          } else {
            target.x = _lpd.x; target.y = _lpd.y;
          }
    ...
    case "warp": {
      ...
      if (_wCursed) {
        ...
        if (FLOOR_MOVE_KINDS.has(kind)) {
          if (kind === "trap" && target.permanent) { ml.push(`${target.name}は動かせない！`); break; }
          const _w1x = target.x + dx, _w1y = target.y + dy;
          if (canStepFloorObjectTo(dg, _w1x, _w1y, kind === "stair" ? null : target, p)) {
            if (kind === "stair") {
              if (!moveStairTo(dg, target, _w1x, _w1y)) { ml.push("テレポートに失敗した。"); break; }
            } else {
              target.x = _w1x; target.y = _w1y;
            }
        ...
        if (stairsX >= 0) {
          if (kind === "monster") {
            ...
            if (_wbAdj.length > 0) {
              const _wd = pick(_wbAdj);
              if (kind === "stair") {
                if (!moveStairTo(dg, target, _wd.x, _wd.y)) { ml.push("テレポートに失敗した。"); break; }
              } else {
                target.x = _wd.x; target.y = _wd.y;
              }
              ...
              if (_wd2) {
                if (kind === "stair") {
                  if (!moveStairTo(dg, target, _wd2.x, _wd2.y)) { ml.push("テレポートに失敗した。"); break; }
                } else {
                  target.x = _wd2.x; target.y = _wd2.y;
                }
                ml.push(`${_flName}はどこかへ飛んだ！`);
              }
      ...
      if (FLOOR_MOVE_KINDS.has(kind)) {
        if (kind === "trap" && target.permanent) { ml.push(`${target.name}は動かせない！`); break; }
        const dest = pickFreeFloorObjectCell(dg, kind === "stair" ? null : target, p, _warpOx, _warpOy);
        if (!dest) { ml.push("テレポートに失敗した。"); break; }
        if (kind === "stair") {
          if (!moveStairTo(dg, target, dest.x, dest.y)) { ml.push("テレポートに失敗した。"); break; }
        } else {
...
export function fireWandBolt(p, dg, eff, dx, dy, ml, luFn, bbFn, blMult = 1, nameFn = null, bigboxNameFn = null) {
  ...
  for (let d = 1; d < MW + MH; d++) {
    ...
    if (it) {
      if (eff === "leap" && blMult >= 1) { _landPlayer(lastX, lastY, _fdx, _fdy); if ((p.immobileTurns||0) > 0) { p.immobileTurns = 0; ml.push("移動封じが解けた！"); } ml.push(`${resolveItemName(it, nameFn)}の前に飛びついた！`); return; }
    ...
    if (stairHit) {
      ...
      applyWandEffect(eff, "stair", stairHit, _fdx, _fdy, dg, p, ml, luFn, bbFn, blMult);
      return;
    }
  ...
  for (let d = 1; d < MW + MH; d++) {
    ...
    if (tx < 0 || tx >= MW || ty < 0 || ty >= MH || dg.map[ty][tx] === T.WALL || dg.map[ty][tx] === T.BWALL) {
      if (killerMon && !consumeBarrier(killerMon, ml)) {
        const _wdmg = multiplyCursedMagicDamage(Math.round(rng(15, 25) * (blessed ? 2 : 1)), killerMon, dg);
        killerMon.hp -= _wdmg;
    ...
    if (tx === pl.x && ty === pl.y) {
      ...
      if (_hasReflect) {
        ...
        if (_srcMon) {
          if (!consumeBarrier(_srcMon, ml)) {
            const _rdmg = multiplyCursedMagicDamage(rng(15, 25), _srcMon, dg);
            _srcMon.hp -= _rdmg;
      ...
      dmg = multiplyCursedMagicDamage(dmg, pl, dg);
    ...
    if (mon) {
      if (monReflectsMagic(mon) && killerMon) {
        ...
        if (!consumeBarrier(killerMon, ml)) {
          const _rdmg = multiplyCursedMagicDamage(rng(15, 25), killerMon, dg);
          killerMon.hp -= _rdmg;
