...
describe("ハーゲンダッ壺とアイスクリーム", () => {
  it("基本から店舗・商品系まで全フレーバーに個別説明がある", () => {
    expect(ICE_CREAM_FLAVORS.length).toBeGreaterThanOrEqual(600);
    ...
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "クリーミーミントアイス")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "クリスピーサンド クアトロフォルマッジ")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "チョコマカデミアアイス")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "ヘブンリースプーン ダージリンアイス")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "紅苺～完熟～アイス")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "クリーミーコーン 鉄観音ミルクティーアイス")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "ソルベ アルフォンソマンゴー")).toBe(true);
    ...
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "やわもちアイス・つぶあんミルク")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "やわもちアイス・ずんだもち")).toBe(false);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "やわもちアイス・わらびもち")).toBe(false);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "パピコ・チョココーヒー")).toBe(true);
    expect(ICE_CREAM_FLAVORS.some((flavor) => flavor.name === "パピコ・ホワイトサワー")).toBe(false);
    ...
    expect(ICE_CREAM_FLAVORS.find((flavor) => flavor.name === "ショートケーキアイス")?.desc).toBeTruthy();
...
describe("water breath ring and soaked", () => {
  it("水の飛沫は敵味方をずぶ濡れにするが、飲んだ水ではならない", () => {
    const dg = {
      map: Array.from({ length: MH }, () => Array(MW).fill(T.FLOOR)),
      monsters: [{ name: "敵", x: 4, y: 3, hp: 20, maxHp: 20, oilyTurns: 5 }],
      oilyTiles: [{ x: 3, y: 3 }, { x: 4, y: 3 }],
      items: [], traps: [], pentacles: [], gachaMachines: [], pendingBombs: [],
    };
    const p = { x: 3, y: 3, hp: 10, maxHp: 20, soakedTurns: 0, oilyTurns: 5, armor: null, inventory: [] };
    const ml = [];
    splashPotion(dg, p.x, p.y, "water", 10, p, ml, () => {});
    expect(p.soakedTurns).toBe(10);
    expect(dg.monsters[0].soakedTurns).toBe(10);
    expect(p.oilyTurns).toBe(0);
    expect(dg.monsters[0].oilyTurns).toBe(0);
    expect(dg.oilyTiles).toEqual([]);
