...
describe("催眠術使い", () => {
  ...
  it("視界内の一直線上にいるプレイヤーへ催眠術をかける", () => {
    const base = MONS.find((m) => m.baseKind === "hypnotist");
    const hypnotist = makeMonsterFromBase(base, 1, 5, 5, { aware: true });
    ...
    expect(messages).toContain("催眠術使いが催眠術をかけた！次の行動を勝手に行ってしまう！");
  });
  ...
  it("魔封じの魔方陣内では催眠術が発動しない", () => {
    ...
    expect(messages).toContain("催眠術使いが催眠術をかけようとしたが、魔封じの魔方陣で封じられた！");
  });
});
