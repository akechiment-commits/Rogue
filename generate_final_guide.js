...
// 同じ状態異常は原因によらず同ターン（対象種別ごと）。祝福アイテムのみ2倍 or 永続。
// ボスは tick 側で半減（isBoss 時 2 減算）。モンスター封印の永続のみボスは 20T 有限。
const statusData = [
  ...
  ['金縛り（paralyzeTurns）', '10', '100（被ダメでも解除）', 'プレイヤー×2／敵は永続', '移動不可・通常回避不可（絶対回避は有効）', '行動不可・通常回避不可（絶対回避は有効）。ボスはtick半減（永続は時間では解けない）', '金縛りの薬・杖・巻物'],
  ['加速（hasteTurns）', '10', '10', '—', '速度×2（2回行動）', '速度UP', '鈍足の杖（呪い）等'],
  ['暗闇（darknessTurns）', '20', '50', '×2／敵は永続', '視界1マス。巻物・魔法書が読めない', '認識不可・直進移動', '暗闇の薬・杖・罠'],
  ['幻惑（bewitchedTurns / fleeingTurns）', '20', '50', '×2／敵は永続', '周囲の見た目がおかしく', 'フロアから逃げ出す', '惑わしの薬・杖・罠'],
  ['魔法封印（sealedTurns）', '50', '永続（ボス20）', '—', '巻物・魔法書・魔法・杖不可', '特技使用不可', '封印の罠・杖'],
  ['MP封印（mpCooldownTurns）', '50', '永続（ボス20）', '×2', 'MP回復不可・魔法使用不可', '特技使用不可', '封印の薬・マナ回復薬（呪い）・封印の杖'],
...
const WAND_DETAILS = {
  ...
  paralyze:     { normal:'敵を100T金縛り（ボスはtick半減・被ダメで解除）', blessed:'敵は永続金縛り（被ダメで解除）／自分は20T', cursed:'対象が200T状態異常免疫を得る' },
  ...
  darkness:     { normal:'敵を50T暗闇 / 自分は20T暗闇（視界1マス）。ボスはtick半減', blessed:'敵は永続暗闇 / 自分は40T暗闇（×2）', cursed:'フロア全体が探索済みになる（透視）' },
  bewitch:      { normal:'敵を50T幻惑（逃げ回る）/ 自分は20T幻惑。ボスはtick半減', blessed:'敵は永続幻惑 / 自分は40T幻惑', cursed:'フロアの罠が全て見える' },
