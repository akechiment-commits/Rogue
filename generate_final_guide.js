...
const specialFloorData = [
  ['フロア', '生成・配置仕様'],
  ['格子の大部屋（gridRoom）', '通路1マス・壁1マスの間隔で柱を交互に配置する。敵・アイテム・罠・泉・大箱を大部屋内へ配置し、稀に壁タイルへ wallEmbedded=true のアイテムを埋め込む。壁を壊すまで通常の床アイテムとしては拾えない。'],
  ['モンスターハウス（通常生成）', '通常フロアではB2Fを含め、テスト用の固定モンスターハウスを生成しない。特殊フロアの通常抽選、モンスターの巻物など個別の発生経路は有効。上級者・超上級者では発生時10%で強モンスターハウスになり、部屋内の全モンスターを1回レベルアップする（Lv3は上限後強化を2回）。祝福されたモンスターの巻物は従来どおり確定で強化する。偽階段の罠はB16F（内部depth=15）以降だけ生成する。'],
];
