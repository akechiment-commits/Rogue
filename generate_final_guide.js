...
const scrollData = [
  ...
  ['雷', '視界内敵 rng(30,40)ダメージ', 'フロア全敵 rng(30,40)ダメージ', '視界内敵 rng(30,40)ダメージ + 自分にも rng(30,40)ダメージ', '725-767'],
  ['回復', '自分と視界内全員がHP+50回復', '自分と視界内全員がHP+100回復', '自分含め視界内全員に35ダメージ', '768-812'],
  ['道具寄せ', 'アイテムを隣接8マスに集める', 'インベントリに直接吸収', 'アイテムをランダム配置', '813-893'],
  ['眠り', '視界内敵6ターン眠り', 'フロア全敵12ターン眠り', '自分含め視界内全員6ターン眠り', '894-920'],
  ['混乱', '視界内敵20ターン混乱', 'フロア全敵40ターン混乱', '自分5ターン混乱 + 視界内敵10ターン混乱', '921-943'],
  ['炎', '視界内敵 rng(30,40)ダメージ（油まみれ×2）', 'フロア全敵 rng(30,40)ダメージ', '視界内敵 rng(30,40) + 自分にも rng(30,40)ダメージ', '944-975'],
  ...
  ['金縛り', '周囲1マスの敵を金縛り', '視界内敵を金縛り', 'プレイヤーが20ターン金縛り', '1035-1060'],
...
const WAND_DETAILS = {
  ...
  lightning:    { normal:'rng(15,25)雷撃ダメ', blessed:'rng(30,50)ダメ（×2）', cursed:'対象HP+25回復（アンデッドには25ダメ）' },
  ...
  sleep:        { normal:'rng(3,5)T眠り', blessed:'rng(6,10)T眠り（×2）', cursed:'フロアの敵が全て見え続ける（透視）' },
  ...
  confuse:      { normal:'敵を20T混乱 / 自分は5T混乱', blessed:'同上', cursed:'同上' },
  ...
  fire_wand:    { normal:'rng(15,25)炎ダメ（油まみれ×2）', blessed:'rng(30,50)ダメ（×2）', cursed:'対象HP最大20回復（アンデッドは20ダメ）' },
