...
const GUIDE_DESC_OVERRIDES = {
  ...
  "願いの杖": "振ると願いUI（文字入力＋選択肢）。願い成功時にchargesを1消費し、0で消滅。\n生成時chargesは常に1。充填の大箱・杖合成では回数増加不可（noChargeBoost）。\n魔封じ中は振っても願い不可（ターン消費のみ）。",
  "願いの壺": "アイテムを1つ入れると、そのアイテムは消滅し壺も消滅して願いUIが開く（1回限り）。\n容量表示は通常の壺同様（rng 3-5）だが、1回の投入で必ず壊れる。\nキャンセル時も壺と捧げたアイテムは戻らない（ターン消費）。",
...
const springData = [
  ...
  ['', '発動時「何を願う？」UI', '文字入力＋固定選択肢（体/知識/運命）'],
  ['', '叶うと泉は必ず干上がる', '1泉あたり最大1願い'],
  ['', 'キャンセル時は干上がらない', '浸す経由はキャンセルでもターン消費'],
  ['', 'テキストでアイテム名を願える', '完全一致/部分一致。最上位合成品は拒否'],
  ['', '選択肢例: 全快・LvUP・全識別・地図・解呪・良アイテム等', 'WISH_PRESETS'],
  ['', '', ''],
