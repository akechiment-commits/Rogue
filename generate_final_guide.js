...
const GUIDE_DESC_OVERRIDES = {
  ...
  "這いずり爆弾": "特殊飛び道具。装備して射撃する場合は1個ずつ、インベントリから「投げる」場合は束全体を1発にまとめて投げ、束の個数を攻撃力へ加算して全個数を消費する。発射後はプレイヤーの行動終了ごとに1マス這い、敵・石像・大箱・プレイヤー・壁に触れるとその地点で地雷と同じ強力な爆発を起こして消滅する。通常の1個投げは爆発半径1（3×3）で、非ボス敵を消滅させ、プレイヤーには現在HPの半分のダメージを与える。束投げは1〜3個で半径1、4〜6個で半径2、7〜9個で半径3、10個以上で半径4（9×9・最大）になる。束投げが2個以上の場合、プレイヤーのHPは半分ではなく1になる。軌道上の罠に乗ると罠を作動させて消費する。壁に向けて撃った場合は発射地点で即爆発する。敵が移動して軌道と交差した場合も接触扱いになる。遠投中や下手投げでは消滅する。",
