...
const scrollData = [
  ...
  ['自爆の巻物', '周囲8マス爆発・自HP→1（炎耐性で半減）・炎無効敵以外即死', '5×5爆発・同効果', '爆発なし・自HP全回復', '新規'],
...
trapData.push(['【共通】', '-', '-', '未発見は踏むと発動。発見済みは歩いても上に乗るだけ（重力の魔方陣下は作動）。回転板は踏むと敵ターン後に発動。']);
...
// 特殊状態（静的）
foodItemData.push(['', '', '', '']);
foodItemData.push(['特殊状態', '腐った食料', '満腹度×0.65 + ダメージrng(5,10) + 毒', '腐敗時に祝福・呪い・壺味・特殊効果のみ除去（大きさは残る）。毒消し指輪で毒は防げる']);
foodItemData.push(['特殊状態', 'ヤバイ食料', '満腹度×0.1 + 大ダメージ + 毒・混乱・幻惑・鈍足', '腐った食料が再腐敗した状態。エンチャントは除去済み']);
foodItemData.push(['', '焼いた食料', '満腹度×2。生食料を炎の薬/炎の杖/加熱の壺で調理', '']);
foodItemData.push(['', '焦げた食料', '満腹度×0.65。調理済みに再度炎が当たる', '']);
