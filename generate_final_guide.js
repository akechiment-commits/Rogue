...
const GUIDE_DESC_OVERRIDES = {
  ...
  "変化の杖": "敵に当てると、現在のフロアで出現する別種の敵へ変化させる。通常は元の敵と同じLv、祝福は1Lv下、呪いは1Lv上へ変化する（Lv1での祝福、Lv3での呪いは同Lvのまま）。壊すと周囲8マスにも同じ効果。自分に跳ね返ると最大HPが-10〜+10変化し、減少時は現在HPも新しい最大HPまで下がる。アイテムは金貨・キー以外をレア度weightで別アイテムにする。C以上だけから抽選する運枠は通常18%、祝福36%、呪い9%。罠は通常・祝福ではレア度weight、呪いでは36%でC以上の罠だけから抽選するため危険な罠に変化しやすい。ボスには効かない。",
...
const statusData = [
  ...
  ['眠り（sleepTurns）', '行動不可・通常回避不可（絶対回避は有効）', '行動不可・通常回避不可（絶対回避は有効）', '睡眠薬・巻物'],
  ['混乱（confusedTurns）', '移動方向がランダム', '攻撃対象がランダム', '混乱薬・巻物'],
  ['移動封じ（immobileTurns）', '移動不可・通常回避不可（絶対回避は有効）', '移動不可・通常回避不可（絶対回避は有効）', '影縫い・氷の杖'],
  ['鈍足（slowTurns）', '速度×0.5・通常回避不可（絶対回避は有効）', '速度×0.5・通常回避不可（絶対回避は有効）', '鈍足の薬・鈍足の杖・鈍足の罠・封印の薬（祝福）'],
  ['金縛り（paralyzeTurns）', 'ターン数の間、移動不可・通常回避不可（絶対回避は有効）', 'ターン数の間、行動不可（詰まり脱出による強制移動も無効）・通常回避不可（絶対回避は有効）', '金縛りの薬・金縛りの杖・金縛りの巻物'],
  ...
  ['凍結（frozenTurns）', '行動不能・通常回避不可（絶対回避は有効）＋受ける物理ダメージ2倍', '（プレイヤーのみ）', '水中/泉上で氷の杖・氷ブレス等を受けたとき（耐氷で無効）'],
];
...
const WAND_DETAILS = {
  ...
  transform:    { normal:'現フロアで出現する別モンスターへ同Lvで変化 / アイテム・罠は通常weight抽選', blessed:'現フロアの別モンスターへ1Lv下げて変化（Lv1なら同Lv） / アイテム運枠36%、罠は通常weight', cursed:'現フロアの別モンスターへ1Lv上げて変化（Lv3なら同Lv） / アイテム運枠9%、罠はC以上運枠36%' },
