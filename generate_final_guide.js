...
const GUIDE_DESC_OVERRIDES = {
  ...
  "変化の杖": "敵に当てると、現在のフロアで出現する別種の敵へ変化させる。通常は元の敵と同じLv、祝福は1Lv下、呪いは1Lv上へ変化する（Lv1での祝福、Lv3での呪いは同Lvのまま）。壊すと周囲8マスにも同じ効果。自分に跳ね返ると最大HPが-10〜+10変化し、減少時は現在HPも新しい最大HPまで下がる。アイテムは金貨・キー以外をレア度weightで別アイテムにする。C以上だけから抽選する運枠は通常18%、祝福36%、呪い9%。罠は祝呪に関わらずレア度weightで別罠に変化する。ボスには効かない。",
...
const WAND_DETAILS = {
  ...
  transform:    { normal:'現フロアで出現する別モンスターへ同Lvで変化 / アイテムはC以上運枠18%で変化', blessed:'現フロアの別モンスターへ1Lv下げて変化（Lv1なら同Lv） / アイテム運枠36%', cursed:'現フロアの別モンスターへ1Lv上げて変化（Lv3なら同Lv） / アイテム運枠9%' },
